...
class scanDetector:
    ...
    def __str__(self):
        ...
        if use_numpy:
            ...
            dimString = "(" + dimString + ")"
        s = (
            "detector <scanRecord>.%s\nPV name  '%s'\nPV desc. '%s'\nPV units    '%s'\ndata: %dD array %s\n"
            % (self.fieldName, self.name, self.desc, self.unit, n, dimString)
        )
        return s
...
def readScan(scanFile, verbose=0, out=sys.stdout, unpacker=None):
    ...
    if (scan.rank > 20) or (scan.rank < 0):
        out.write(
            "* * * readScan('%s'): rank > 20.  Probably a corrupt file\n"
            % (scanFile.name)
        )
        return (None, None)
...
def readScanQuick(scanFile, unpacker=None, detToDat_offset=None, out=sys.stdout):
    ...
    if (scan.rank > 20) or (scan.rank < 0):
        out.write(
            "* * * readScanQuick('%s'): rank > 20.  Probably a corrupt file\n"
            % (scanFile.name)
        )
        return None
    ...
    for j in range(scan.np):
        ...
        if n:
            u.set_position(u.get_position() + 4 + (n + 3) // 4 * 4)
    ...
    if (detToDat_offset is None) or (not useDetToDatOffset):
        ...
        if diff != 0:
            out.write(
                "oldSeek=0x%x, newSeek=0x%x, o-n=%d\n"
...
def readMDA(
    ...
    if verbose:
        out.write("verbose=%d output for MDA file '%s'\n" % (verbose, fname))
    ...
    if verbose:
        out.write("MDA version = %.3f\n" % version)
    if (abs(version - 1.3) > 0.01) and (abs(version - 1.4) > 0.01):
        out.write(
    ...
    if verbose:
        out.write("isRegular = %d\n" % isRegular)
    pExtra = u.unpack_int()
    if verbose:
        out.write("pExtra = %d (0x%x)\n" % (pExtra, pExtra))
    ...
    if (rank > 1) and (maxdim > 1):
        ...
        for i in range(dim[0].curr_pt):
            if dim[0].plower_scans[i] == 0:
                if verbose:
                    out.write(
                        "1D point %d/%d; declining to seek null file loc; appending zeros\n"
                        % (i, dim[0].curr_pt)
                    )
                # append empty data arrays.  If i==0; this won't work.
                numP = len(dim[1].p)
                for j in range(numP):
                    dim[1].p[j].data.append([0] * len(dim[1].p[j].data[0]))
                numD = len(dim[1].d)
                for j in range(numD):
                    dim[1].d[j].data.append([0] * len(dim[1].d[j].data[0]))
            else:
                scanFile.seek(dim[0].plower_scans[i])
                if verbose:
                    out.write(
                        "1D point %d/%d; seek = 0x%x\n"
                ...
                if i == 0:
                    ...
                    if s.np > numP:
                        out.write(
                            "First scan had %d positioners; This one only has %d.\n"
                            % (len(dim[1].p), s.np)
                        )
                    for j in range(numP):
                    ...
                    if s.nd > numD:
                        out.write(
                            "First scan had %d detectors; This one only has %d.\n"
                            % (len(dim[1].d), s.nd)
                        )
                    for j in range(numD):
    ...
    if (rank > 2) and (maxdim > 2):
        ...
        for i in range(dim[0].curr_pt):
            ...
            if dim[0].plower_scans[i] == 0:
                if verbose:
                    out.write(
                        "1D point %d/%d; declining to seek null file loc\n"
                        % (i, dim[0].curr_pt)
                    )
                # append empty data arrays.  If i==0, this won't work
                s1 = dim[2]
                ...
                for j in range(s1.curr_pt):
                    ...
                    if s1.plower_scans[j] == 0:
                        if verbose:
                            out.write(
                                "2D point [%d,%d]/[%d,%d]; declining to seek null file loc; appending zeros\n"
                        ...
                        for k in range(numP):
                            ...
                            dim[2].p[k].data[i].append(
                                [0] * len(dim[2].p[k].data[0][0])
                            )
                        numD = len(dim[2].d)
                        for k in range(numD):
                            if j == 0:
                                dim[2].d[k].data.append([])
                            dim[2].d[k].data[i].append(
                                [0] * len(dim[2].d[k].data[0][0])
                            )
                    else:
                        scanFile.seek(s1.plower_scans[j])
                        if verbose:
                            out.write(
                                "2D point %d/%d; seek = 0x%x\n"
            ...
            if dim[0].plower_scans[i] == 0:
                ...
                for j in range(s1.curr_pt):
                    if s1.plower_scans[j] == 0:
                        if verbose:
                            out.write(
                                "2D point [%d,%d]/[%d,%d]; declining to seek null file loc\n"
                                % (i, j, dim[0].curr_pt, s1.curr_pt)
                            )
                    else:
                        scanFile.seek(s1.plower_scans[j])
                        (s2, detToDat) = readScan(
                            scanFile, max(0, verbose - 1), out, unpacker=u
                        )
                        for k in range(s2.curr_pt):
                            if s2.plower_scans[k] == 0:
                                if verbose:
                                    out.write(
                                        "3D point %d/%d; declining to seek null file loc\n"
                                        % (k, s2.curr_pt)
                                    )
                            else:
                                ...
                                if verbose:
                                    out.write(
                                        "4D point [%d,%d,%d]/[%d,%d,%d]; seek = 0x%x\n"
                                        % (
                                            i,
                                            j,
                                            k,
                                ...
                                if (i == 0) and (j == 0) and (k == 0):
                                    ...
                                    if j == 0 and k == 0:
                                        ...
                                        for m in range(dim[3].np):
                                            if k == 0:
                                                dim[3].p[m].data[i].append([])
                                            dim[3].p[m].data[i][j].append(s.p[m].data)
                                        for m in range(dim[3].nd):
                                            if k == 0:
                                                dim[3].d[m].data[i].append([])
    ...
    if showHelp:
        ...
        print(
            "      name - name of scan record that acquired this dimension: '%s'"
            % (dim[1].name)
        )
        print(
...
def skimScan(dataFile):
    ...
    if (scan.rank > 20) or (scan.rank < 0):
        print(
            "* * * skimScan('%s'): rank > 20.  probably a corrupt file" % dataFile.name
        )
        return None
    scan.npts = u.unpack_int()
    scan.curr_pt = u.unpack_int()
...
def skimMDA(fname=None, verbose=False):
    ...
    if rank > 1:
        ...
        if dim[1]:
            ...
            if verbose:
                print("had a problem reading 2d from ", fname)
            return None
    ...
    if rank > 2:
        ...
        if dim[2]:
            ...
            if verbose:
                print("had a problem reading 3d from ", fname)
            return None
    ...
    if rank > 3:
        ...
        if dim[3]:
            ...
            if verbose:
                print("had a problem reading 4d from ", fname)
            return None
...
def packScanHead(scan):
    ...
    p.pack_int(n)
    if n:
        p.pack_string(scan.name)
    n = len(scan.time)
    p.pack_int(n)
    if n:
        p.pack_string(scan.time)
    p.pack_int(scan.np)
    p.pack_int(scan.nd)
    p.pack_int(scan.nt)
    ...
    for j in range(scan.np):
        ...
        p.pack_int(n)
        if n:
            p.pack_string(scan.p[j].name)

        n = len(scan.p[j].desc)
        p.pack_int(n)
        if n:
            p.pack_string(scan.p[j].desc)

        n = len(scan.p[j].step_mode)
        p.pack_int(n)
        if n:
            p.pack_string(scan.p[j].step_mode)

        n = len(scan.p[j].unit)
        p.pack_int(n)
        if n:
            p.pack_string(scan.p[j].unit)

        n = len(scan.p[j].readback_name)
        p.pack_int(n)
        if n:
            p.pack_string(scan.p[j].readback_name)

        n = len(scan.p[j].readback_desc)
        p.pack_int(n)
        if n:
            p.pack_string(scan.p[j].readback_desc)

        n = len(scan.p[j].readback_unit)
        p.pack_int(n)
        if n:
            p.pack_string(scan.p[j].readback_unit)
    ...
    for j in range(scan.nd):
        ...
        p.pack_int(n)
        if n:
            p.pack_string(scan.d[j].name)
        n = len(scan.d[j].desc)
        p.pack_int(n)
        if n:
            p.pack_string(scan.d[j].desc)
        n = len(scan.d[j].unit)
        p.pack_int(n)
        if n:
            p.pack_string(scan.d[j].unit)
...
def opMDA_usage():
    ...
    print("\n examples:")
    print(
        "   r = opMDA('+', scan1, scan2) -- adds all detector data from scan1 and scan2"
    )
    print(
        "   r = opMDA('-', scan1, 2.0)   -- subtracts 2 from all detector data from scan1"
    )
            ...
            for k in range(s[2].npts):
                for l in range(s[3].npts):
                    for m in range(s[4].npts):
                        s[4].d[i].data[j][k][l][m] = op(
                            s[4].d[i].data[j][k][l][m], scalar
                        )
...
def opMDA(op, d1, d2):
    ...
    """
    if isScan(d1) and isScalar(d2):
        return opMDA_scalar(op, d1, d2)
    if not isScan(d1):
    ...
    for i in range(s[3].nd):
        for j in range(s[1].npts):
            for k in range(s[2].npts):
                s[3].d[i].data[j][k] = map(
                    op, s[3].d[i].data[j][k], d2[3].d[i].data[j][k]
                )
    ...
    for i in range(s[4].nd):
        for j in range(s[1].npts):
            for k in range(s[2].npts):
                for l in range(s[3].npts):
                    s[4].d[i].data[j][k][l] = map(
                        op, s[4].d[i].data[j][k][l], d2[4].d[i].data[j][k][l]
                    )
